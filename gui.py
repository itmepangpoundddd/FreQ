...
# Title + Artist
    ...
    def _open_mic_filters(self) -> None:
        """OBS-style per-stage filter dialog for live mic segments."""
        ...
        dialog.minsize(380, 340)
        dialog.transient(self)
        dialog.grab_set()
        ctk.CTkLabel(dialog, text="Mic filter chain (applied live)",
                     font=ctk.CTkFont(size=13, weight="bold")).pack(pady=(14, 2))
        ctk.CTkLabel(dialog,
                     text="Every stage runs natively in C++ per mic.\n"
                          "Order is fixed: Suppression → Compressor → Limiter.",
                     font=ctk.CTkFont(size=9), text_color=COLORS["text3"],
                     justify="center").pack()
        rows = []
        descriptions = {
            "suppression": "High-pass + noise gate (rumble, hiss, room, fans)",
            "compressor": "Evens loud/soft phrases (broadcast voice balance)",
            "limiter": "Safety ceiling — stops accidental clipping",
            "deesser": "Tames sharp S sounds (~5 kHz band only)",
        }
        for stage, label in self._MIC_FILTER_LABELS:
            ...
            seg = ctk.CTkSegmentedButton(
                row, values=["Gentle", "Strong"], variable=strength)
            seg.pack(side="right")
            hint = ctk.CTkLabel(dialog, text=descriptions[stage],
                                font=ctk.CTkFont(size=9),
                                text_color=COLORS["text3"], anchor="w",
                                justify="left")
            hint.pack(fill="x", padx=30)
            rows.append((stage, var, strength))

        def apply_filters() -> None:
            for stage, var, strength in rows:
                if not var.get():
                    mode = 0
                else:
                    mode = 2 if strength.get() == "Strong" else 1
                self.audio_player.set_mic_filter(stage, mode)
            self._refresh_mic_filters_status()
            dialog.destroy()

        ctk.CTkButton(dialog, text="Apply", command=apply_filters,
                      fg_color=COLORS["green"],
                      text_color="#000000").pack(side="bottom", pady=(6, 14))
