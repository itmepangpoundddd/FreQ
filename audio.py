...
class AudioDeviceManager:
    ...
    def _refresh(self) -> None:
        ...
        # Capture endpoints: native WASAPI enumeration ONLY. PortAudio
        # must never enumerate or open microphones — some old USB webcam
        # mics crash the whole process inside its DLL (0xC0000005), and
        # its device indices are useless to the native engine anyway.
        try:
        ...
        if _nac is not None:
            try:
                for idx, mic in enumerate(_nac.enum_microphones()):
                    ep_id, name, _is_default = mic
                    self._devices.append(AudioDevice(
                        id=idx,
                        name=name or f"Microphone {idx}",
                        host_api="WASAPI",
                        max_input_channels=2,
                        max_output_channels=0,
                        default_sample_rate=48000,
                        endpoint_id=ep_id or None,
                    ))
            except Exception:
                pass
            ...
            try:
                ...
                for i, d in enumerate(raw):
                    ...
                    if not is_output:
                        continue  # inputs are listed natively above
                    host_api = ""
                    try:
