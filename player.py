...
class AudioPlayer:
    ...
    # Hard ceiling of simultaneously live capture endpoints mixed into
    # one output stream (matches the engine's kMaxMics).
    MAX_LIVE_MICS = 4
    ...
    def __init__(self) -> None:
        ...
        # Master broadcast chain (0926.2 P1): (mode, bass, mid, treble,
        # comp_thresh_db, comp_ratio_x10, limit_ceiling_db) — default OFF.
        self._master_chain: tuple = (0, 0.0, 0.0, 0.0, -24.0, 30, -1.0)
        ...
        self._mic_session = 0   # generation counter for live-mic sessions
        # Dedicated render engine for live mics when the MUSIC engine is
        # pygame: mics must always be real-time, music keeps its path.
        self._mic_render = None
        self._mic_render_module = None
        # Live-mic pause bookkeeping (wall-clock accounting for the
        # clip timer extension on resume).
        self._mic_pause_started = 0.0
        self._mic_pause_total = 0.0
    ...
    def shutdown(self) -> None:
        ...
        if self._mic_render is not None:
            try:
                self._mic_render_module.render_stop(self._mic_render)
            except Exception:
                pass
            self._mic_render = None
            self._mic_render_module = None
    ...
    def set_master_chain(self, mode: int, bass: float = 0.0,
                         mid: float = 0.0, treble: float = 0.0,
                         comp_thresh_db: float = -24.0, comp_ratio_x10: int = 30,
                         limit_ceiling_db: float = -1.0) -> None:
        """Push the master chain (EQ → Compressor → Limiter) to the engine.

        mode 0 = OFF (legacy bytes), 1 = gentle, 2 = strong. EQ in dB
        (−12…+12), compressor threshold dBFS (−48…−6), ratio ×10
        (10…80), limiter ceiling dBFS (−6…0, <0 = limiter engaged).
        """
        self._master_chain = (int(mode), float(bass), float(mid),
                              float(treble), float(comp_thresh_db),
                              int(comp_ratio_x10), float(limit_ceiling_db))
        self._apply_master_chain()

    def get_master_chain(self):
        return tuple(getattr(self, "_master_chain",
                             (0, 0.0, 0.0, 0.0, -24.0, 30, -1.0)))

    def _apply_master_chain(self) -> None:
        if self._engine == "wasapi" and self._render is not None:
            try:
                nar = self._render_module
                mode, b, m, t, ct, cr, cd = self._master_chain
                nar.render_master_set(self._render, mode, b, m, t,
                                      ct, cr, cd)
            except Exception:
                logging.getLogger("freq.player").exception(
                    "render_master_set failed")
    ...
    def _apply_eq(self) -> None:
        ...
        if self._engine == "wasapi" and self._render is not None:
            try:
                ...
                self._apply_master_chain()   # chain state survives engine swaps
            except Exception:
    ...
    def use_wasapi_engine(self, endpoint_id: str = "") -> bool:
        ...
        # The music engine just took over as the mic host — retire any
        # live-mic session still riding the DEDICATED mic engine, or it
        # would keep mixing there while the next session mixes here
        # (voice doubled). Only after the switch SUCCEEDED, so a failed
        # switch leaves everything untouched.
        if self._mic_render is not None:
            ...
            self._mic_pause_total = 0.0
            try:
                self._mic_render_module.render_emergency_mic_stop(
                    self._mic_render)
            except Exception:
                pass
            try:
    ...
    def pause(self) -> None:
        ...
        with self._lock:
            if self._is_playing and not self._is_paused:
                # Live mic session: the music path pauses its own engine
                # below; the mic mix itself is paused in the engine that
                # hosts the mics (music engine OR dedicated mic engine).
                if self.emergency_mic_active():
                    self._pause_offset = self.get_position()
                    self._is_paused = True
                    self._mic_pause_started = time.monotonic()
                    for module, engine in ((self._render_module, self._render),
                                           (self._mic_render_module, self._mic_render)):
                        if module is not None and engine is not None:
                            try:
                                module.render_emergency_mic_pause(engine, True)
                            except Exception:
                                pass
                    if self._engine == "wasapi" and self._render is not None:
                        try:
    ...
    def resume(self) -> None:
        """Resume a paused mic session and/or the music underneath."""
        if not self.available:
            return
        with self._lock:
            if self._is_paused:
                if self._mic_pause_started > 0:
                    # Extend the clip timer by the paused duration so the
                    # segment plays its FULL length, not minus pause time.
                    self._mic_pause_total += time.monotonic() - self._mic_pause_started
                    self._mic_pause_started = 0.0
                    for module, engine in ((self._mic_render_module, self._mic_render),
                                           (self._render_module, self._render)):
                        if module is not None and engine is not None:
                            try:
                                module.render_emergency_mic_pause(engine, False)
                            except Exception:
                                pass
                if self._engine == "wasapi":
    ...
    # ─────────────────────────────────────
    def _mic_engine_pair(self):
        """(module, render engine) for live mics — never None on success.

        The music engine (WASAPI) hosts the mics when it is active;
        otherwise a dedicated tiny render engine is created once and
        reused, so mic segments are ALWAYS real-time even while music
        plays through pygame. Music behaviour is never touched.
        """
        if self._engine == "wasapi" and self._render is not None:
            return self._render_module, self._render
        if self._mic_render is not None:
            return self._mic_render_module, self._mic_render
        try:
            import native_audio_render as nar
        except ImportError:
            return None, None
        if not self._initialized:
            self.init()
        render = nar.render_new()
        try:
            nar.render_start(render, None)   # OS default output endpoint
        except Exception as e:
            print(f"  WARNING mic engine start failed: {e}")
            return None, None
        self._mic_render = render
        self._mic_render_module = nar
        # A late-created engine must start with the remembered chain.
        for stage, (setter, _getter) in self._MIC_FILTER_APIS.items():
            mode = self._mic_filters().get(stage, 0)
            if mode:
                try:
                    getattr(nar, setter)(render, mode)
                except Exception:
                    pass
        return nar, render

    def play_mic_multi(self, duration: float, specs,
                       duck_factor: float = 0.3,
                       on_finish: Optional[Callable] = None) -> int:
        """Open several LIVE mics at once; return how many actually opened.

        specs: iterable of (endpoint_id_or_None, gain). The FIRST entry is
        the primary: it owns the session (duck, timeline, timer and the
        on_finish callback). Later entries JOIN that session — they may
        never reset the timer chain or re-duck, or the queue would stall
        when the clip ends. Each mic is its own capture + FIFO in the
        engine: one failing never affects the others.
        """
        specs = list(specs) or [(None, 1.0)]
        nar, render = self._mic_engine_pair()
        if nar is None or render is None:
            return 0
        opened = 0
        try:
            if not nar.render_is_running(render):
                return 0
            # A fresh session SUPERSEDES any still-live one (rapid
            # re-press, or a queued segment while the emergency mic is
            # sticky). Retire old mics FIRST — capacity is then always
            # available and the old session's timer/callback cannot
            # outlive its own mics. The old timer, if armed, dies via
            # the event (or the session-id guard on a tight race).
            self._mic_live_stop.set()
            try:
                nar.render_emergency_mic_stop(render)
            except Exception:
                pass
            # Engine capacity: four live mics is the hard ceiling (their
            # audio is summed in one device-buffer pass). Only reachable
            # when mics were opened OUTSIDE a session (direct engine use).
            room = self.MAX_LIVE_MICS - self.mic_count()
            if room <= 0:
                logging.getLogger("freq.player").warning(
                    "live mic capacity reached (%d)", self.MAX_LIVE_MICS)
                return 0
            if duck_factor > 0.0 and duck_factor < 1.0                     and self._mic_duck_restore is None:
                prev = self._volume
                self._volume = prev * duck_factor
                self._apply_volume()
                self._mic_duck_restore = prev
            for endpoint, gain in specs[:room]:
                try:
                    nar.render_emergency_mic(render,
                                             endpoint or None, gain)
                    opened += 1
                except Exception:
                    logging.getLogger("freq.player").exception(
                        "mic %r refused (gain %.2f) — continuing",
                        endpoint, gain)
            if opened == 0:
            ...
            self._restore_mic_duck()
            logging.getLogger("freq.player").exception(
                "multi-mic start failed")
            return 0
        # A mic segment IS the programme: when nothing else is playing,
        # give the UI a working timeline (position/duration) so the
        # progress bar advances. When a song still plays underneath, its
        # timeline is kept — the music never stopped.
        if not self._is_playing:
            self._is_playing = True
            self._is_paused = False
            self._current_file = None
            ...
            self._pause_offset = 0.0
            self._play_start_time = time.time()
        self._mic_pause_started = 0.0   # fresh session: no pause debt
        self._mic_pause_total = 0.0
        ...
        self._mic_live_thread = threading.Thread(
            target=self._mic_live_timer,
            args=(max(0.0, duration), session), daemon=True)
    ...
    def play_mic_live(self, duration: float, device_endpoint: str = "",
                      ...
                      on_finish: Optional[Callable] = None) -> bool:
        """Single live mic — thin wrapper over play_mic_multi."""
        return self.play_mic_multi(
            duration, [(device_endpoint or None, mic_gain)],
            duck_factor=duck_factor, on_finish=on_finish) > 0
    ...
    def _mic_live_timer(self, duration: float, session: int = 0) -> None:
        if duration <= 0:
            ...
            return
        # Wait out the clip, honouring pause time: the timer is extended
        # by whatever the transport spent paused, so the segment plays
        # its FULL length. Poll in short slices — resume() extends
        # _mic_pause_total while we sleep.
        end = time.monotonic() + duration
        while True:
            remaining = end + self._mic_pause_total - time.monotonic()
            if remaining <= 0:
                break
            if self._mic_live_stop.wait(min(remaining, 0.2)):
                return   # transport stopped the mic — no finish callback
        if session and session != self._mic_session:
            return   # superseded by a newer session — not ours to end
    ...
    def stop_emergency_mic(self) -> None:
        ...
        self._mic_live_thread = None
        self._mic_live_on_finish = None
        # Mics may live in the music engine OR the dedicated mic engine —
        # stop them wherever they are.
        for module, engine in ((self._render_module, self._render),
                               (self._mic_render_module, self._mic_render)):
            if module is not None and engine is not None:
                try:
                    module.render_emergency_mic_stop(engine)
                except Exception:
                    pass
        # If the mic owned the transport (no song underneath) and no new
        # session took over, end the "playing" state — otherwise the
        # progress bar would freeze mid-way after toggling the mic off.
        if self._is_playing and self._current_file is None:
            self._is_playing = False
        self._restore_mic_duck()
    ...
    def emergency_mic_active(self) -> bool:
        for module, engine in ((self._render_module, self._render),
            ...
            if module is not None and engine is not None:
                try:
                    if module.render_emergency_mic_active(engine):
                        return True
                except Exception:
                    pass
        return False
    ...
    def mic_count(self) -> int:
        """Number of live capture endpoints currently mixed in."""
        total = 0
        for module, engine in ((self._render_module, self._render),
                               (self._mic_render_module, self._mic_render)):
            if module is not None and engine is not None:
                try:
                    total += int(module.render_emergency_mic_count(engine))
                except Exception:
                    pass
        return total

    # Mic filter chain (OBS-style, ALL native C++): stage -> 0 off /
    # 1 gentle / 2 strong. Remembered so engines created LATER (mic
    # engine on the pygame path, engine switches) start the same way.
    _mic_filter_modes: dict = None

    def _mic_filters(self) -> dict:
        if self._mic_filter_modes is None:
            self._mic_filter_modes = {"suppression": 0, "compressor": 0,
                                      "limiter": 0, "deesser": 0,
                                      "miceq": 0, "echo": 0,
                                      "gainst": 0, "upcomp": 0,
                                      "expand": 0}
        return self._mic_filter_modes

    _MIC_FILTER_APIS = {
        # stage -> (setter name, getter name) on native_audio_render
        "suppression": ("render_emergency_mic_suppression",
                        "render_emergency_mic_get_suppression"),
        "compressor": ("render_emergency_mic_compressor",
                       "render_emergency_mic_get_compressor"),
        "limiter": ("render_emergency_mic_limiter",
                    "render_emergency_mic_get_limiter"),
        "deesser": ("render_emergency_mic_deesser",
                    "render_emergency_mic_get_deesser"),
        "miceq": ("render_emergency_mic_miceq",
                  "render_emergency_mic_get_miceq"),
        "echo": ("render_emergency_mic_echo",
                 "render_emergency_mic_get_echo"),
        "gainst": ("render_emergency_mic_gainst",
                   "render_emergency_mic_get_gainst"),
        "upcomp": ("render_emergency_mic_upcomp",
                   "render_emergency_mic_get_upcomp"),
        "expand": ("render_emergency_mic_expand",
                   "render_emergency_mic_get_expand"),
    }

    def set_mic_filter(self, stage: str, mode: int) -> None:
        """Set one native filter stage (0 off / 1 gentle / 2 strong)."""
        api = self._MIC_FILTER_APIS.get(stage)
        if api is None:
            return
        try:
            mode = int(mode)
        ...
        if mode < 0 or mode > 2:
            return
        self._mic_filters()[stage] = mode
        setter, _getter = api
        for module, engine in ((self._render_module, self._render),
                               (self._mic_render_module, self._mic_render)):
            if module is not None and engine is not None:
                try:
                    getattr(module, setter)(engine, mode)
                except Exception:
                    pass

    def set_mic_noise_suppression(self, mode: int) -> None:
        """Back-compat alias -> the suppression chain stage."""
        self.set_mic_filter("suppression", mode)

    def mic_filters(self) -> dict:
        """Current remembered chain {stage: mode} (engine-independent)."""
        return dict(self._mic_filters())

    # Per-stage fine parameters (OBS-style sliders) — generic key/value
    # bridge into the native engine. Values are ints; the UI labels units.
    _MIC_PARAM_KEYS = (
        "sup_hp_hz", "sup_gate_db", "gate_floor_pct", "gain_db",
        "comp_db", "comp_ratio10", "lim_db", "de_hz", "de_db",
        "eq_low_db", "eq_high_db", "echo_ms", "echo_fb_pct",
        "echo_wet_pct", "up_db", "up_ratio10", "ex_db", "ex_depth_pct",
    )

    def mic_param_get(self, key: str) -> int:
        for module, engine in ((self._render_module, self._render),
                               (self._mic_render_module, self._mic_render)):
            if module is not None and engine is not None:
                try:
                    return int(module.render_mic_param_get(engine, key))
                except Exception:
                    continue
        return -1
    ...
    def mic_param_set(self, key: str, value: int) -> bool:
        ...
        for module, engine in ((self._render_module, self._render),
            ...
            if module is not None and engine is not None:
                try:
                    module.render_mic_param_set(engine, key, int(value))
                    applied = True
                except Exception:
                    continue
        return applied

    def mic_noise_suppression(self) -> int:
        return self._mic_filters()["suppression"]
