...
def test_native_aircheck_stream_modules() -> None:
    """New native modules (0926.2 base): WAV-writer and live-stream senders.

    Structural: the C++ sources implement the real paths (WAV header patch,
    source-header send, reconnect ladder) and release.py verifies + bundles
    both .pyd files so they can never silently drop out of a release.
    """
    sources = {}
    for name in ("native_aircheck.cpp", "native_stream.cpp"):
        try:
            sources[name] = (Path(__file__).parent / name).read_text(
                encoding="utf-8")
        except OSError as err:
            check(f"[native-0926] {name} readable", False, str(err))
            return
    rel_src = (Path(__file__).parent / "release.py").read_text(
        encoding="utf-8")
    build_src = (Path(__file__).parent / "build_native_meter.py").read_text(
        encoding="utf-8")
    try:
        cpp = (Path(__file__).parent / "native_audio_render.cpp").read_text(
            encoding="utf-8")
    except OSError as err:
        check("[native-0926] native_audio_render.cpp readable", False, str(err))
        return
    try:
        py_src = (Path(__file__).parent / "player.py").read_text(
            encoding="utf-8")
    except OSError as err:
        check("[native-0926] player.py readable", False, str(err))
        return

    ac = sources["native_aircheck.cpp"]
    for token, name in (
        ("aircheck_start", "recorder start API"),
        ("aircheck_stop", "recorder stop API"),
        ("PatchWaveHeader", "WAV sizes patched on stop (crash-safe file)"),
        ("AUDCLNT_STREAMFLAGS_LOOPBACK : 0", "loopback OR mic selectable"),
        ("g_zombie", "stuck-device leak policy like native_audio_capture"),
    ):
        check(f"[native-0926] aircheck {name}", token in ac)

    st = sources["native_stream.cpp"]
    for token, name in (
        ("stream_start", "stream start API"),
        ("ConnectSink", "TCP sink with source-header send"),
        ("StartEncoder", "ffmpeg stdin/stdout plumbing"),
        ("kReconnecting", "reconnect state machine"),
        ("reconnect_delay_ms", "ladder delay honoured from caller"),
        ("SO_SNDTIMEO", "stalled server cannot hang the thread"),
        ("TCP_NODELAY", "small-frame latency kept low"),
        ("TerminateProcess", "encoder owned + killed on stop"),
    ):
        check(f"[native-0926] stream {name}", token in st)

    # Live-mix wiring: streaming.py wrapper + GUI switch (0926.2)
    try:
        stream_src = (Path(__file__).parent / "streaming.py").read_text(
            encoding="utf-8")
        gui_src = (Path(__file__).parent / "gui.py").read_text(
            encoding="utf-8")
    except OSError as err:
        check("[native-0926] streaming.py readable", False, str(err))
        return
    for token, name in (
        ("class NativeMixStream", "live-mix wrapper class exists"),
        ("def _ffmpeg_path", "encoder resolved from bundled ffmpeg first"),
        ("ns.stream_state() == 2", "start() waits for real STREAMING state"),
        ("def update_metadata", "ICY metadata push via admin API"),
        ("if song == self._meta_song:", "metadata dedupe (no spam per poll)"),
        ("_native_mix = NativeMixStream()", "module-level singleton exported"),
        ('name="freq-icy-meta"', "ICY metadata pushed from a named worker"),
        ("def _push()", "metadata HTTP hop wrapped in a background fn"),
    ):
        check(f"[native-0926] wrapper {name}", token in stream_src)
    for token, name in (
        ("self.stream_native_var = ctk.BooleanVar(value=False)",
         "Live-mix switch exists, default OFF"),
        ("if self.stream_native_var.get():", "Start routes to native path"),
        ("def _start_native_stream", "native start handler"),
        ("def _push_native_metadata", "queue -> ICY metadata handler"),
        ("_native_mix.update_metadata(song.title, song.artist)",
         "metadata pushed on every track change"),
        ("if _native_mix.active:", "stop tears down the native stream"),
        ('"live_mix": bool(self.stream_native_var.get())',
         "switch persisted in .freq"),
        ('self.stream_native_var.set(bool(s.get("live_mix", False)))',
         "switch restored from .freq"),
        # ── Auto air-check on air (0926.2 P3) ──
        ('"auto_onair": bool(self.aircheck_auto_var.get())',
         "auto air-check persisted in .freq"),
        ('self.aircheck_auto_var.set(bool(acr.get("auto_onair", False)))',
         "auto air-check restored from .freq"),
        ("self._safe_poll(self._aircheck_auto_poll, \"aircheck_auto\")",
         "auto air-check driven by the 500 ms tick"),
        ("def _aircheck_native_start", "native recorder start helper"),
        ("def _aircheck_native_stop", "native recorder stop + MP3 helper"),
        ("if rec is not None and rec.recording:",
         "manual/scheduled recordings take priority over auto"),
        ("na.aircheck_start(path, \"\", True)",
         "auto recording captures the FULL mix (loopback)"),
        ("AirCheckRecorder.export_mp3(path, delete_wav=True)",
         "auto stop honours the MP3 export setting"),
        ("AirCheckRecorder.clean_old(",
         "auto stop prunes old recordings per keep-days"),
        ("def _aircheck_auto_finish_worker",
         "save+MP3+prune run on a WORKER thread (UI never blocks)"),
        ("threading.Thread(target=self._aircheck_auto_finish_worker",
         "poll hands off to the background worker"),
        ('return getattr(self, "_mic_session_open", False)',
         "on-air mic hint fallback when the engine call is unavailable"),
        # ── Master Broadcast Chain (0926.2 P1) ──
        ("def _open_master_chain", "master chain dialog exists"),
        ('text="⚙ Configure Master Chain…"',
         "dialog reachable from the settings page"),
        ("self.audio_player.set_master_chain(",
         "dialog commits through the player bridge"),
        ('"master_chain": {', "chain persisted in .freq"),
        ('mch = settings.get("master_chain", {})',
         "chain restored from .freq"),
    ):
        check(f"[native-0926] gui {name}", token in gui_src)

    # Master chain DSP: C++ implements the 3 stages + the API, the bridge
    # pushes it (and re-pushes after engine swaps), release.py verifies it.
    for token, name in (
        ("void master_process(int16_t* samples, size_t total)",
         "master DSP implemented in the render engine"),
        ("if (mode == 0 && !eq_active && !limit_active)",
         "all-OFF = byte-identical legacy path"),
        ("PyObject* render_master_set", "set API"),
        ("PyObject* render_master_get", "get API"),
        ("PyObject* render_master_selftest", "DSP self-test without device"),
        ("master_process(dst_as_int16(dst), avail * kChannels)",
         "chain runs on the FINAL mix (after mic mixing)"),
        ("master_process(dst_as_int16(sdst), space * kChannels)",
         "chain runs in the empty-ring/mic-only path too"),
        ("master_process(dst_as_int16(dst), space * kChannels)",
         "chain runs while paused (voice-over-paused-music)"),
        ("std::pow(thr_lin / env, 1.0 - 1.0 / ratio)",
         "compressor gain math (downward, ratio-aware)"),
    ):
        check(f"[native-0926] master-dsp {name}", token in cpp)
    for token, name in (
        ("self._apply_master_chain()   # chain state survives engine swaps",
         "player re-pushes chain after every engine swap"),
        ("def _apply_master_chain", "player bridge apply"),
    ):
        check(f"[native-0926] master-py {name}", token in py_src)
    for token, name in (
        ("render_master_set", "master chain verified by release pipeline"),
        ("render_master_get", "master get verified by release pipeline"),
        ("render_master_selftest", "self-test verified by release pipeline"),
    ):
        check(f"[native-0926] release {name}", token in rel_src)

    for token, name, in_rel, in_build in (
        ("NATIVE_AIRCHECK_FUNCTIONS", "aircheck verified by release pipeline", True, False),
        ("NATIVE_LIVESTREAM_FUNCTIONS", "stream verified by release pipeline", True, False),
        ('("native_aircheck", NATIVE_AIRCHECK_FUNCTIONS)',
         "aircheck in BOTH verify passes (build + bundled payload)", True, False),
        ('("native_stream", NATIVE_LIVESTREAM_FUNCTIONS)',
         "stream in BOTH verify passes (build + bundled payload)", True, False),
        ('"native_aircheck", "native_stream"):',
         "both .pyd bundled into the installer payload", False, True),
    ):
        target = rel_src if in_rel else build_src
        check(f"[native-0926] release {name}", token in target)
...
def main() -> None:
    ...
    # Combo soak (0926.2): Go Live + auto air-check coexistence guard
    try:
        from test_combo_aircheck_stream import run_all as _combo_run_all
        RESULTS.append(("[combo] suite importable", True, ""))
    except ImportError as _e:
        RESULTS.append(("[combo] suite importable", False, str(_e)))
    test_native_aircheck_stream_modules()
    print(f"\n=== Summary: {engine} ===")
