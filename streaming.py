...
# ── Native live-mix bridge (native_stream.cpp, 0926.2) ────────────────────

class NativeMixStream:
    """Broadcast the LIVE master mix through the native C++ sender.

    Wraps ``native_stream`` (loopback → ffmpeg → Icecast/SHOUTcast in one
    C++ thread). The legacy ``FFmpegStreamer`` paths stream FILES; this
    class streams what the app is actually playing right now — live mics,
    filters and all — which is what listeners must hear.
    """

    STATE_NAMES = {0: "Idle", 1: "Connecting", 2: "Streaming",
                   3: "Reconnecting", 4: "Stopped", 5: "Error"}
    ...
    def available() -> bool:
        try:
            import native_stream  # noqa: F401
            return True
        except ImportError:
            return False
    ...
    # -- control ----------------------------------------------------------
    def start(self, host: str, port: int, mount: str, username: str,
              password: str, stream_name: str, bitrate: int = 128,
              reconnect_delay_ms: int = 2000) -> bool:
        """Open the native live-mix stream. Returns True once STREAMING."""
        if not self.available():
            return False
        import native_stream as ns
        if ns.stream_state() in (1, 2, 3):
            return True   # already live
        mount = mount if mount.startswith("/") else f"/{mount}"
        import base64
        auth = base64.b64encode(
            f"{username or 'source'}:{password}".encode()).decode()
        try:
            rate = 48000
            try:
                import native_audio_capture as _nac
                rate = _nac.Capture(loopback=True).samplerate
            except Exception:
                pass
            ffmpeg = _ffmpeg_path()
            if not ffmpeg:
            ...
            enc = (f'"{ffmpeg}" -hide_banner -loglevel error '
                   ...
                   f"-c:a libmp3lame -b:a {bitrate}k -f mp3 pipe:1")
            header = (
                f"SOURCE {mount} ICY/1.0\r\n"
                f"Authorization: Basic {auth}\r\n"
                f"User-Agent: FreQ\r\n"
                f"Content-Type: audio/mpeg\r\n"
                f"ice-name: {stream_name}\r\n\r\n"
            )
            ns.stream_start(host, int(port), header, enc,
                            int(reconnect_delay_ms))
        except (OSError, ValueError, RuntimeError):
            return False
        # wait briefly for the C++ thread to reach STREAMING
        deadline = time.time() + 8.0
        while time.time() < deadline:
            if ns.stream_state() == 2:
                self._active = True
                self._cfg = {"host": host, "port": int(port), "mount": mount,
                             "username": username or "source",
                             "password": password}
                return True
            if ns.stream_state() == 5:
                break
            time.sleep(0.1)
        return False
    ...
    def status(self) -> dict:
        """(state_name, sent_bytes, attempts, last_error) snapshot."""
        if not self.available():
            return {"state": "Unavailable", "sent": 0, "attempts": 0, "error": ""}
        import native_stream as ns
        st = ns.stream_state()
        sent, att, _rate, err = ns.stream_stats()
        return {"state": self.STATE_NAMES.get(st, str(st)), "sent": sent,
                "attempts": att, "error": err}

    # -- ICY metadata (Icecast admin API, same as the legacy path) --------
    def update_metadata(self, title: str, artist: str = "") -> None:
        """Push ICY metadata from a BACKGROUND thread.

        The admin-API round-trip can block up to the 5 s socket timeout
        (unreachable server) — it must NEVER run on the Tk UI thread or
        every track change could freeze the whole app.
        """
        if not (self._active and self._cfg):
            return
        song = f"{artist} - {title}".strip(" -")
        if song == self._meta_song:
            return
        self._meta_song = song
        cfg = dict(self._cfg)
        ...
        def _push() -> None:
            try:
                ...
                import base64
                params = urlencode({"mode": "upinfo",
                                    "mount": cfg["mount"], "song": song})
                url = (f"http://{cfg['host']}:{cfg['port']}"
                       f"/admin/metadata?{params}")
                auth = base64.b64encode(
                    f"{cfg['username']}:{cfg['password']}".encode()).decode()
                urlopen(Request(url,
                                headers={"Authorization": f"Basic {auth}"}),
                        timeout=5)
            except Exception as e:  # non-critical by design
                logger.debug(f"Live-mix metadata update failed: {e}")

        threading.Thread(target=_push, daemon=True,
                         name="freq-icy-meta").start()


_native_mix = NativeMixStream()
