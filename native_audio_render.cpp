...
private:
    ...
    void emergency_mic_loop(MicBlock* m) {
        ...
        while (!m->zombie.load() && mic_is_member(m)) {
            ...
            UINT32 packet = 0;
            while (m->capture->GetNextPacketSize(&packet) == S_OK && packet > 0) {
                if (m->zombie.load() || !mic_is_member(m)) break;
                BYTE* data = nullptr;
                UINT32 frames = 0;
                DWORD flags = 0;
    ...
    void mix_emergency_mic(BYTE* dst, size_t frames) {
        ...
        // Noise-suppression tuning @ 44.1 kHz (kept literal so the
        // bypass path stays untouched when dsp_mode == 0).
        //  gentle: HPF ~80 Hz, gate opens -45 dBFS, floor -12 dB
        //  strong: HPF ~120 Hz, gate opens -38 dBFS, floor -24 dB, faster
        const float hp_c      = dsp_mode == 2 ? 0.0189f : 0.0126f;
        const float gate_thr  = dsp_mode == 2 ? 0.0126f : 0.0056f;
        const float gate_floor= dsp_mode == 2 ? 0.0625f : 0.25f;
        ...
        // De-esser tuning (split-band around ~5 kHz).
        const float de_lp_c    = de_mode == 2 ? 0.60f : 0.50f; // ~5–6.3 kHz split
        const float de_thr     = de_mode == 2 ? 0.06f : 0.10f;
        ...
        // Compressor tuning (threshold, ratio, follower, smoothing).
        const float comp_thr     = comp_mode == 2 ? 0.032f : 0.063f;
        const float comp_ratio   = comp_mode == 2 ? 6.0f : 3.5f;
        ...
        // Limiter tuning (ceiling, release, smoothing).
        const float lim_ceiling = lim_mode == 2 ? 0.85f : 0.95f;
        ...
        while (running_.load()) {
            ...
            if (was_paused) {
                ...
                if (device_fails >= kMaxDeviceFails) {
                    running_.store(false);   // device lost — engine down
                    break;
                }
